...
async function runTests() {
  ...
  // Test 2: Simulate webhook expired for a bill
  console.log('\nTest 2: Simulate webhook expired handling...');
  ...
  if (!targetBill) {
    console.log('⚠ No paid bill found to test expired handling');
  } else {
    // Get completed payment for this bill
    ...
    if (!targetPayment) {
      console.log('⚠ No payment found for bill');
    } else {
      console.log(`  Testing expired on bill ${targetBill.id.slice(0, 8)}...`);
      console.log(`  - Payment ${targetPayment.id.slice(0, 8)}... status=${targetPayment.status} amount=${targetPayment.amount}`);

      // Simulate expired webhook: mark payment as failed
      ...
      if (updatePaymentError) {
        console.log(`  ✗ Failed to update payment: ${updatePaymentError.message}`);
      } else {
        ...
        const newBillStatus = totalPaid >= Number(targetBill.amount) ? 'paid' : totalPaid > 0 ? 'partial' : 'pending';
        ...
        if (updateBillError) {
          console.log(`  ✗ Failed to update bill: ${updateBillError.message}`);
        } else {
          console.log(`  ✓ Payment marked as failed`);
          console.log(`  ✓ Bill status updated: ${newBillStatus}`);
        }
      }
    }
  }

  // Test 3: Verify all paid bills have valid supporting payments
  console.log('\nTest 3: Verify all paid bills have valid supporting payments...');
  ...
  // Test 4: Verify pending/partial bills don't have completed payments
  console.log('\nTest 4: Verify pending/partial bills have correct payment states...');
  ...
  // Test 5: Check payment_gateway_transactions consistency
  console.log('\nTest 5: Check payment_gateway_transactions consistency...');
