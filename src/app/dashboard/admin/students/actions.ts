...
export async function getStudentsAction(
  ...
  if (searchQuery && searchQuery.trim()) {
    ...
    query = query.or(`nis.ilike.${escaped},full_name.ilike.${escaped}`);
  }
  ...
  if (studentsError) {
    console.error("[getStudentsAction Error]", studentsError);
...
export async function importStudentsAction(formData: FormData) {
  ...
  const isDapodik =
    headers.length === DAPODIK_HEADERS.length &&
    headers.every((h, idx) => h === DAPODIK_HEADERS[idx]);
  const isSimple =
    headers.length === SIMPLE_HEADERS.length &&
    headers.every((h, idx) => h === SIMPLE_HEADERS[idx]);

  if (!isDapodik && !isSimple) {
    return {
      error: `Header tidak valid. Yang diharapkan:\n- Format Dapodik: ${DAPODIK_HEADERS.join(
        ", "
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!nis || !fullName) {
        results.push({
          ...
          full_name: fullName || "-",
          status: "error",
          message: "NIS dan Nama Lengkap harus diisi",
        });
        continue;
      }
    ...
    if (upserts.length > 0) {
      ...
      for (const student of updates) {
        ...
        results.push({
          ...
          full_name: student.full_name,
          status: "updated",
          message: "NIS sudah terdaftar, data diperbarui",
        });
      }
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (!trimmedNipdNis || !trimmedFullName) {
      results.push({
        ...
        status: "error",
        message: "NIS/NIPD dan Nama Lengkap harus diisi",
      });
      continue;
    }
  ...
  const allNisnValues = upserts
    .map((u) => u.nisn)
    .filter((nisn): nisn is string => Boolean(nisn));
  ...
  if (upserts.length > 0) {
    ...
    if (existingByNisResult.error) {
      return {
        error: `Gagal memeriksa data siswa yang sudah ada: ${existingByNisResult.error.message}`,
      };
    }
    if (existingByNisnResult.error) {
      return {
        error: `Gagal memeriksa data siswa yang sudah ada: ${existingByNisnResult.error.message}`,
      };
    }

    const existingStudents = [
      ...(existingByNisResult.data || []),
      ...(existingByNisnResult.data || []),
    ];
    const existingByNis = new Map((existingStudents || []).map((s) => [s.nis, s]));
    const existingByNisn = new Map(
      (existingStudents || []).filter((s) => s.nisn).map((s) => [s.nisn, s])
    );

    const inserts: DapodikStudentUpsert[] = [];
    const updates: {
      existing: { id: string; nis: string };
      update: DapodikStudentUpsert;
      class_name: string | null;
    }[] = [];

    for (const student of upserts) {
      const existing =
        existingByNis.get(student.nis) ||
        (student.nisn ? existingByNisn.get(student.nisn) : undefined);
      if (existing) {
    ...
    for (const item of updates) {
      ...
      results.push({
        row: 0,
        nis: item.update.nis,
        full_name: item.update.full_name,
        status: "updated",
    ...
    const insertedIds: {
      ...
      full_name: string;
      class_name?: string | null;
      data?: DapodikStudentUpsert | null;
    }[] = [];
    if (inserts.length > 0) {
      const { error: insertError } = await supabase.from("students").insert(
      ...
      if (insertedError) {
        return {
          error: `Gagal memuat data siswa yang baru ditambahkan: ${insertedError.message}`,
        };
      }
      ...
      for (const s of insertedStudents || []) {
        ...
        insertedIds.push({
          ...
          class_name: original?.class_name || null,
          data: original || null,
        });
      }
    }
    ...
    for (const item of allProcessed) {
      const studentId =
        typeof item.existingId === "string" && item.existingId.length > 20
          ? item.existingId
          : null;
      const studentNis =
        typeof item.existingId === "string" && item.existingId.length <= 20
      ...
      async function findOrCreateParent(
        ...
        const baseEmail = studentNis
          ? `${studentNis}@sekolah.id`
          : `${roleLabel}_${finalStudentId}@placeholder.local`;
        let email = baseEmail;
            ...
            if (existingAuthUser?.id) {
              ...
              if (!existingName || existingName === trimmedName) {
                ...
                if (altAuthError || !altAuthData?.user) {
                  console.error(
                    `Failed to create alternative ${roleLabel} account`,
                    altAuthError
                  );
                  return null;
                }
      ...
      if (academicYearId && item.class_name) {
        ...
        if (classId) {
          const studentStatus = String(
            (item.data as DapodikStudentUpsert).status || ""
          ).toLowerCase();
...
export async function getStudentAction(studentId: string | undefined) {
  if (!studentId) {
    return { error: "ID siswa tidak valid." };
  }
  const supabase = await createClient();
  ...
  const normalizedEnrollments = (enrollments || []).map((enrollment: StudentEnrollmentRow) => {
    const classItem = Array.isArray(enrollment.classes)
      ? enrollment.classes[0]
      : enrollment.classes;
    const yearItem = Array.isArray(enrollment.academic_years)
